...
class TicTacToe:
    def __init__(self, root):
        ...
        self.board_size = 3  # Change board size here
    ...
    def on_button_click(self, row, col):
        if self.board[row * self.board_size + col] == "":
            ...
            if self.check_winner():
                ...
                self.remove_random_tokens()  # Remove random tokens
                self.update_buttons()
    ...
    def check_winner(self):
        for i in range(self.board_size):
            if all(self.board[i + j * self.board_size] == self.current_player for j in range(self.board_size)):
                return True
        ...
        if all(self.board[i * (self.board_size + 1)] == self.current_player for i in range(self.board_size)):
            return True
        if all(self.board[(self.board_size - 1) * (i + 1)] == self.current_player for i in range(self.board_size)):
            return True
        return False
